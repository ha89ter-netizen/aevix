...
test.describe("политика описывает поведение, а не намерение", () => {
  test("не обещает, что до входа ничего не уходит на сервер", async ({ page }) => {
    /**
     * Утверждение «на наш сервер это не уходит» было неправдой: не входя в аккаунт, лендинг
     * отправляет введённый текст на `/api/business-analysis` и дальше в OpenAI. Документ обязан
     * описывать то, что приложение ДЕЛАЕТ, — иначе он не защищает никого, а только вводит в
     * заблуждение.
     *
     * Проверяется не наличие нужной формулировки (её можно переписать), а отсутствие обещания,
     * которое код нарушает.
     */
    await page.goto("/privacy");
    const text = (await page.locator("main").innerText()).replace(/\s+/g, " ");

    expect(text, "обещание, которого код не выполняет").not.toContain("На наш сервер это не уходит");
    // И отправка должна быть названа прямо — умолчание здесь тоже форма неправды.
