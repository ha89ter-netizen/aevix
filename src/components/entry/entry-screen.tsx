  ...
  return (
    <div className="entry-screen" data-surface="marketing">
      ...
      {/*
        ...
        Правовые ссылки — исключение, и оно не отменяет сказанного выше. Это не разделы продукта,
        а документы, которые обязаны быть достижимы с любой публичной поверхности: у экрана есть
        свой адрес, на него приходят по прямой ссылке, и человек не должен уходить на другую
        страницу, чтобы узнать, что происходит с его данными. Двух ссылок для этого достаточно —
        перечня разделов здесь по-прежнему нет.
      */}
      <footer className="entry-base">
        <span className="entry-base-rule" aria-hidden="true" />
        <nav className="entry-base-legal" aria-label={t("entry.nav.legal")}>
          <Link href="/privacy">{t("entry.legal.privacy")}</Link>
          <Link href="/terms">{t("entry.legal.terms")}</Link>
        </nav>
