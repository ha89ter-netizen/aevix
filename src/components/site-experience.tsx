...
export function HeroAnalyzer() {
  ...
  /**
   * Чип подставляет пример в поле — и на этом останавливается.
   *
   * Раньше он сразу запускал разбор, и это давало разбор ВЫДУМАННОГО бизнеса: в чипе лежит
   * шаблон («барбершоп на 3 мастера, запись веду вручную»), а не описание того, кто нажал. У
   * человека с пятью мастерами и записью в Instagram получался чужой сценарий, поданный как его
   * собственный, — при том, что вся страница просит рассказать про СВОЙ бизнес.
   *
   * Теперь текст ложится в поле, курсор встаёт в конец, и его можно поправить под себя, прежде
   * чем нажать «Разобрать». Одно нажатие превратилось в два — но второе принадлежит человеку, а
   * не случайности.
   */
  const pickExample = (example: string) => {
    ...
    requestAnimationFrame(() => {
      ...
      if (!el) return;
      el.focus();
      // Курсор в конец, а не в начало: следующее действие — дописать своё, а не читать чужое.
