import type { Dictionary } from "./ru";

/** Английский. Ключи обязаны совпадать с русским словарём — за этим следит тип `Dictionary`. */
export const en: Dictionary = {
  "entry.meta.title": "AEVIX — an operating system for business",

  "entry.nav.home": "Back to the AEVIX entry screen",
  "entry.nav.language": "Interface language",
  "entry.nav.languageScope": "Entry screen only for now",

  "entry.hero.eyebrow": "An operating system for business",
  "entry.hero.headline": "Your business runs.\nThe system thinks.",
  "entry.hero.lead":
    "AEVIX brings analysis, website, processes and pricing into one workspace — and keeps them in agreement as the business grows.",
  "entry.hero.primary": "Open website",
  "entry.hero.secondary": "Sign in / Register",

  "entry.scene.name": "Process map",

  "entry.nav.legal": "Legal documents",
  "entry.legal.privacy": "Privacy",
  "entry.legal.terms": "Terms",

  "eco.aria.scene": "Living map of AEVIX capabilities. Use arrow keys to move between them.",
  "eco.cap.ai": "AI assistant",
  "eco.cap.website": "Website",
  "eco.cap.analytics": "Analytics",
  "eco.cap.payments": "Payments",
  "eco.cap.bookings": "Bookings",
  "eco.cap.reviews": "Reviews",
  "eco.cap.automation": "Automation",
  "eco.cap.calendar": "Calendar",
  "eco.cap.support": "Support",
  "eco.cap.email": "Email",
  "eco.cap.knowledge": "Knowledge",
  "eco.cap.inventory": "Inventory",
  "eco.cap.marketing": "Marketing",
};
