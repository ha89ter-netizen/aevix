...
export function LocaleProvider({ children }: { children: ReactNode }) {
   ...
   *
   * Возврат прежнего значения на размонтировании обязателен, как и у `lang` выше: провайдер
   * обёрнут вокруг ОДНОГО экрана, и переход на `/platform` без этого оставил бы английский
   * заголовок на русской странице.
   */
  useEffect(() => {
    const previous = document.title;
    document.title = DICTIONARIES[locale]["entry.meta.title"] || ru["entry.meta.title"];
    return () => {
