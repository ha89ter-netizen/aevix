...
/**
 * Строка о скидке уходит в промпт AI-консультанта дословно, поэтому она обязана совпадать с тем,
 * что человек увидит в расчёте. Одно число и одна база: процент считается от стоимости выбранной
 * конфигурации — той самой, что показана как «старая цена».
 */
export const FIRST_PROJECT_DISCOUNT_LINE = `на первый проект действует реальная скидка ${Math.round(
  FIRST_PROJECT_DISCOUNT * 100,
)}% от стоимости выбранной конфигурации — называй её при вопросах о цене, это настоящее условие, а не маркетинговый приём`;
